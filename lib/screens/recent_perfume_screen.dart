...
class _RecentPerfumeScreenState extends State<RecentPerfumeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<RecentPerfumeProvider>().loadRecent();
    });
  }

  @override
  Widget build(BuildContext context) {
    final recent = context.watch<RecentPerfumeProvider>().recentPerfumes;

    // ✅ 3개씩 묶어서 행 단위로 구성
    final List<List<Map<String, String>>> rows = [];
    for (int i = 0; i < recent.length; i += 3) {
      rows.add(recent.sublist(i, (i + 3 > recent.length) ? recent.length : i + 3));
    }
