...
class _PBTIQuestionScreenState extends State<PBTIQuestionScreen> {
  final List<String> questions = [
    ...
    '앰버·우드·레진처럼 따뜻하고 포근한 향이 더 만족스럽다', // 역
    '비나 바다를 떠올리는 물기 있는 향이 좋다',
    '가을·겨울에 어울리는 따뜻한 무드의 향을 더 선호한다', // 역
    '그린·허브의 시원하고 선명한 느낌이 마음에 든다',
    '스모키·스위트한 따뜻함이 느껴지는 향에 더 마음이 간다', // 역

    // 질감 축 (Light ↔ Heavy)
    '가까이에서만 은은하게 느껴지는 가벼운 향이 좋다',
    '분사 후 존재감이 강하고 밀도 있는 향을 선호한다', // 역
    '상쾌한 첫인상이 지속력보다 더 중요하다',
    '하루 종일 진한 잔향이 남아야 만족한다', // 역
    '적게 뿌려도 부담 없는 가벼운 질감을 선호한다',
    '무게감과 깊이가 느껴지는 진한 질감이 매력적이다', // 역

    // 무드 축 (Sweet ↔ Spicy)
    '바닐라·카라멜·초콜릿 같은 달콤한 향이 좋다',
    '후추·시나몬·카다멈 등 향신료의 톡 쏘는 느낌을 더 좋아한다', // 역
    '복숭아·베리·사과 등 과일의 달큰함이 매력적이다',
    '달달한 향은 금방 물리는 편이다', // 역
    '허브·생강 같은 드라이하고 깔끔한 스파이스 무드가 좋다', // 역
    '디저트 숍을 연상시키는 포근한 단향이 편안하다',

    // 자연성 축 (Natural ↔ Modern)
    '숲·풀·흙처럼 자연의 냄새에 끌린다',
    '오존·레더·머스크 등 현대적·합성 느낌이 세련되게 느껴진다', // 역
    '에센셜 오일 기반의 자연스러운 향을 선호한다',
    '세제/샤워젤 같은 깨끗한 합성 향을 특히 좋아한다', // 역
    '전통적/자연주의 향취가 실험적 향보다 더 마음에 든다',
    '합성적이더라도 창의적이고 니치한 향이면 더 끌린다', // 역
  ];
  ...
  // 정확한 역문항 인덱스 (0-based)
  final List<int> reversedIndexes = [
    1, 3, 5,   // 온도
    7, 9, 11,  // 질감
    13, 15, 16, // 무드
    19, 21, 23 // 자연성
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: answeredCount == questions.length
                    ? () {
                  // 1) null → 0 치환
                  final clean = answers.map((e) => e ?? 0).toList();

                  // 2) 역문항 점수 반전
                  for (int i in reversedIndexes) {
                    if (clean[i] != 0) clean[i] = 6 - clean[i];
                  }
                  ...
                  // 4) 결과 Map 생성
                  final result = {
                    '온도': avg(clean.sublist(0, 6)),
                    '질감': avg(clean.sublist(6, 12)),
                    '무드': avg(clean.sublist(12, 18)),
                    '자연성': avg(clean.sublist(18, 24)),
                  };
                  ...
                  Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                      builder: (_) => PBTILoadingScreen(result: result),
                    ),
                  );
                }
                    : null,
                ...
                child: const Text('분석하기', style: TextStyle(color: Colors.white)),
              ),
            ),
