    ...
    def detect(self, img_bgr, guide_box: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        ...
        if self.yolo is not None:
            try:
                ...
                # 패스1: bottle만
                res = run_pt(640, classes=[39])
                # 패스2: 유사 클래스 추가
                if res.boxes is None or len(res.boxes) == 0:
                    res = run_pt(960, classes=[39, 40, 41, 75])
                # 패스3: 클래스 제한 해제
                if res.boxes is None or len(res.boxes) == 0:
                    res = run_pt(1280, classes=None, conf=0.03)
        ...
        try:
            ...
            x0, y0 = unletterbox(x, y); x1, y1 = unletterbox(x + bw, y + bh)
            ...
            return {"present": True, "score": 1.0, "mask_polygon": npoly,
